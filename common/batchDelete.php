...
if($step == '1')
{
	...
	$smarty->display('BatchDeleteView1.tpl');
}
elseif($step == '2')
{

	$deleteAssociateby = false;
	if(isset($_POST['delete_associateby_record']) && $_POST['delete_associateby_record'] == 'YES')
	{
	...
	$_SESSION[_SESSION_KEY]['listview_batch_delete_params'][$module]['deleteAssociateby'] = $deleteAssociateby;
	
	$count = $mod->batchDelete($queryWhere,$filterWhere,$orderby,$order,$userids,$groupids,$limit,$deleteAssociateby,true);
	$smarty->assign('MODULE_LABEL',getTranslatedString($module));
	$smarty->assign('DELETE_COUNT',$count);
	$smarty->display('BatchDeleteView2.tpl');
}
elseif($step == '3')
{
	...
	{	
		...
		$deleteAssociateby=$_SESSION[_SESSION_KEY]['listview_batch_delete_params'][$module]['deleteAssociateby'] ;
		$count = $mod->batchDelete($queryWhere,$filterWhere,$orderby,$order,$userids,$groupids,$limit,$deleteAssociateby,false);
		$smarty->assign('MODULE_LABEL',getTranslatedString($module));
		$smarty->assign('DELETE_COUNT',$count);
		$smarty->display('BatchDeleteView3.tpl');		
	}
	else
	{
