<?php
class TelemarketingSHWYModule extends BaseModule
{
	public $baseTable = 'accounts_shwy';
	//模块描述
	public $describe = '电销业务(上海唯佑专用)';
	//需要控制访问权限的模块方法
	...
	public $fields = Array(
			...
			'company'          => Array('27','S',false,'保险公司',0),
			...
			'vin'              => Array('5','S',false,'车架号/车辆识别代码'),
			...
			'expiration_date'  => Array('31','DT',false,'保险到期日期'),
	...
	public function getOnePreset(){
		...
		$sql = "SELECT id FROM {$this->baseTable} WHERE user_attach = {$CURRENT_USER_ID} ";
		$sql .= "AND DATE_FORMAT(preset_time,'%Y-%m-%d %H:%i') BETWEEN ";
		$sql .= "CONCAT(DATE_SUB(DATE_FORMAT(CURRENT_TIMESTAMP(),'%Y-%m-%d'),INTERVAL 10 DAY),' 00:00') ";
		$sql .= "AND DATE_FORMAT(CURRENT_TIMESTAMP(),'%Y-%m-%d %H:%i') AND status <> 'FAILED' AND status <> 'INVALID' AND status <> 'SUCCESS' ORDER BY preset_time ASC LIMIT 0,1";
	...
	public function getNextPreset(){
		global $APP_ADODB,$CURRENT_USER_ID;
		$sql = "SELECT id FROM {$this->baseTable} WHERE user_attach = {$CURRENT_USER_ID} ";
		$sql .= "AND DATE_FORMAT(preset_time,'%Y-%m-%d %H:%i') >= DATE_FORMAT(CURRENT_TIMESTAMP(),'%Y-%m-%d %H:%i') ";
	...
	public function getOneFirstDial(){
		global $APP_ADODB,$CURRENT_USER_ID;
		$sql = "SELECT id FROM {$this->baseTable} WHERE user_attach = {$CURRENT_USER_ID} ";
		$sql .= "AND TIME_TO_SEC(preset_time) = 0 AND status = 'FIRST_DIAL' ORDER BY id ASC LIMIT 0,1";
		$result = $APP_ADODB->Execute($sql);
		if($result->RecordCount()) return $result->fields[0];
		return 0;
	}
	 ...
	 * @return [Int] [成功返回记录ID,失败返回0]
	 */
	public function getNextOneRecordsetID(){
