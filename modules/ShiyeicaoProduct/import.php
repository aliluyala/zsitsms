...
elseif($step == "2")
{
    ...
    $smarty->display('Accounts/Import2.tpl');
}
elseif($step == "3" )
{
    if(empty($_GET['datafile']))
    ...
    $smarty->assign('IMPORT_TYPE',$_POST['import_type']);
    ...
    $smarty->display('Accounts/Import3.tpl');
...
elseif($step == 4)
{
    ...
    if(!empty($cols)&&($fext == 'xls' || $fext == 'xlsx'))
    {
        ...
        for($line=$startline;$line<=$maxRows;$line++)
        {
            ...
            if($_POST['import_type'] == 'common'){
                $recordid =  getNewModuleSeq($mod_table);
                $mod->insertOneRecordset($recordid,$data,TRUE) ? $insert_row++ : $ignore_row++;
            }else{
                $info = $mod->getAccountByVin($data['vin']);
                if($info){
                    $mod->updateOneRecordset($info['id'],
                             $CURRENT_IS_ADMIN?NULL:getRecordset2UsersPermission($module),
                             $CURRENT_IS_ADMIN?NULL:getRecordset2GroupsPermission($module),
                             $data,TRUE) ? $update_row ++ : $ignore_row++;
                }else{
                    $recordid =  getNewModuleSeq($mod_table);
                    $mod->insertOneRecordset($recordid,$data,TRUE) ? $insert_row++ : $ignore_row++;
                }
    ...
    @unlink($datafile);
    $smarty->assign('INSERT_ROW',$insert_row);
    $smarty->assign('IGNORE_ROW',$ignore_row);
    $smarty->assign('UPDATE_ROW',$update_row);
    $smarty->assign('DATA_LINES',$maxRows);
    $smarty->display('Accounts/Import4.tpl');
}
?>