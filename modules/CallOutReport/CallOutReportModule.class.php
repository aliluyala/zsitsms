<?php
class CallOutReportModule extends BaseModule
{
	...
	//记录权限关联字段名
	public $shareField = 'us.id';//cdr.userid
	...
	public function getListQueryRecord($queryWhere,$filterWhere,$orderby,$order,$userids,$groupids,$start,$maxRows){
		...
		$sql = "SELECT cdr.*{$addFields} FROM {$this->baseTable} cdr LEFT JOIN users us ON cdr.agent_name = us.agent_workno LEFT JOIN groups gs ON gs.id = us.groupid {$where} group by agent_name,dir {$having} {$sqlorder} limit {$start},{$maxRows};";
		$result = $APP_ADODB->Execute($sql);
