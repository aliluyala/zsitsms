<?php
class HandOutModule extends BaseModule
{
	...
	function setDistributeId2Temporary(){
		...
		$sql = "CREATE TEMPORARY TABLE cosmo(index_id INT PRIMARY KEY AUTO_INCREMENT,id INT,vin CHAR(50)) AS SELECT id,vin FROM {$this->baseTable} {$where}";
	...
	function getDistributeIdFromTemporary($idString){
		global $APP_ADODB;
		$sql = "SELECT id,vin FROM cosmo WHERE index_id IN ({$idString})";
	 ...
	 * @return [array]           [执行结果]
	 */
	function recycleData($accarray,$sitarray,$vinarray,$num){
		global $CURRENT_USER_ID;
	...
	function run($option){
		...
		if($option == 'handout'){
			...
			return $this->recycleData($accarray,$sitarray,$vinarray,$customer_num);
