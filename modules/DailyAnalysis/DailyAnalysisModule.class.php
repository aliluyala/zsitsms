<?php
class DailyAnalysisModule extends BaseModule
{
	...
	public function setDailyAnalysis($recordid,$status_new){
		...
		}elseif(($status_old == "APPOINTMENT_QUOTATION" || $status_old == "APPOINTMENT_NON_QUOTATION")){
	...
	public function getStatus($recordid){
		...
		$sql = "SELECT status FROM accounts WHERE id = {$recordid}";
