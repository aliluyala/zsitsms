...
if($user_type == '用户')
{
	...
	if($role_type == 'PRO')
	{
		...
		if($pmws->addProcessManager($user_id,0,$role_guid))
		{
	...
	elseif($role_type == 'TAS')
	{
		...
		if($pmws->addTaskUser($user_id,0,$role_guid))
		{
			return_ajax('success','成功');			
		}
		else
...
elseif($user_type == '工作组')
{
	if($role_type == 'PRO')
	{
		if($pmws->addProcessManager($user_id,1,$role_guid))
		{
			return_ajax('success','成功');
	...
	elseif($role_type == 'TAS')
	{
		if($pmws->addTaskUser($user_id,1,$role_guid))
		{
			return_ajax('success','成功');
