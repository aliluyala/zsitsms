...
//设置模板路径
$tpl_file        = 'CallOutTrendChart/ListView.tpl';
...
if($list_data){
    foreach($list_data as $val){
        $create_date .= "'" . $val['create_date']['value'] . "',";
        $num         .= $val['num']['value'] . ",";
        $num_valid   .= $val['num_valid']['value'] . ",";
        $total_time  .= $val['total_time']['value'] . ",";
    }
    $create_date = substr($create_date, 0,-1);
    $num         = substr($num, 0,-1);
    $num_valid   = substr($num_valid, 0,-1);
...
$smarty->assign('LISTVIEW_DATA_CREATE_DATE',$create_date);
$smarty->assign('LISTVIEW_DATA_NUM',$num);
$smarty->assign('LISTVIEW_DATA_NUM_VALID',$num_valid);
$smarty->assign('LISTVIEW_DATA_TOTAL_TIME',$total_time);
...
global $tpl_file;
if(!isset($tpl_file)) $tpl_file = 'ListView.tpl';
