...
<div id="{$MODULE}_search_dlg" title="搜索...">
	<div id="search_ui_list" style="display:none">
		...
		<div id="search_ui_condition_date">
...
<script>//增加搜索条件
	function zswitch_add_search_conditin_modify(mod)
	{
		...
		selField.change(function(){
			...
			else if(search_ui[fieldName]["dataType"] == "DT")
			{
				thistr.children("td:eq(1)").html(search_uis.children("#search_ui_condition_date").html());
			}
			else
			{
	...
	$(".listview_search_button").button().click(function(){ldelim}
		...
		{ldelim}
			zswitch_add_search_conditin_modify("{$MODULE}");
		{rdelim}
