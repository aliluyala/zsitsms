...
<div style="margin-top:3px;">
    <div id="entry-post-list" style="padding:1px;">
        ...
        {/if}
        <div id="entry-post-category">
            <ul class="nav-category">
                <li><a href="javascript:MenuEntry(0,'')">首页</a></li>
            {if $POST_CATEGORY}
                {foreach from=$POST_CATEGORY item=category}
                    <li>
                        {if $category.list neq FALSE}
                            <span class="ui-menu-icon ui-icon ui-icon-carat-1-e rightarrow"></span>
                        {/if}
                        <a href="javascript:MenuEntry({$category.id},'{$category.name}')">{$category.name}</a>
                        {if $category.list neq FALSE}
                        <dl class="nav-subcategory">
                            <dt><span class="genericon genericon-leftarrow arrow"></span></dt>
                            {foreach from=$category.list item=subcategory}
                                <dd><a href="javascript:MenuEntry({$subcategory.id},'{$subcategory.name}')">{$subcategory.name}</a></dd>
                            {/foreach}
                        </dl>
                            <!-- <ul class="nav-subcategory">
                                <li id="arrow"><span class="test"></span></li>
                            {foreach from=$category.list item=subcategory}
                                <li><a href="javascript:zswitch_load_client_view('index.php?module=LibraryPost&action=index&category={$subcategory.id}')">{$subcategory.name}</a></li>
                            {/foreach}
                            </ul> -->
                        {/if}
                    </li>
...
<script type="text/javascript" >
    ...
    {literal}
        ...
        function MenuEntry(menuId,label){
            search_query_where = new Array();
            if(menuId != 0){
                search_query_where[0]    = new Array();
                search_query_where[0][0] = 'categoryid';
                search_query_where[0][1] = '=';
                search_query_where[0][2] = menuId;
                search_query_where[0][3] = '';
                search_query_where[0][4] = label;
            }
            $("#client_listview_table_form").children("[name=query_where]").val($.toJSON(search_query_where));
            $("#client_listview_table_form").children("[name=record_page]").val("1");
            var url = "index.php?module=" + listModuleName + "&action=index";
            zswitch_load_client_view(url,"client_listview_table_form");
        }
        $("div#entry-post-category dl.nav-subcategory").css("left",($("div#entry-post-category").width() + "px"));
    {/literal}
</script>
