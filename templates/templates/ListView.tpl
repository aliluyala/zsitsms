		...
		<tr>
			...
			{foreach $row as $fieldname => $field}
				...
				{if $field.have_associate}
					<a href="{$field.associate_to}" recordid="{$id}" title="{$field.title}">{$field.value}</a>
				{else}
					{$field.value}
				{/if}
				&nbsp;
...
<script type="text/javascript" >
	...
	{literal}
		//防止在页数输入框中按下回车时触发第一个按钮事件
		$(function(){
			$("input[name=current_page]").on("keydown",function(event){
				if(event.keyCode == 13){
					zswitch_client_listview_page_ctrl($(this).value,'client_listview_table_form');
					return false;
				}
			});
		});
