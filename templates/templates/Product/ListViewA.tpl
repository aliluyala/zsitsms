...
<div class="client_listview_bar small">
	<table border="0" cellspacing="0"  width="100%">
		<tr>
			...
			{/if}
			{if $LISTVIEW_BUTTONS.entry_repertory }
				<button class="client_listview_button small" title="入库" action="entry_repertory">入库</button>
			{/if}
			{if $LISTVIEW_BUTTONS.out_repertory }
				<button class="client_listview_button small" title="出库" action="out_repertory">出库</button>
			{/if}			
			</td>
...
<div id="entry_repertory_box"></div>
<div id="out_repertory_box"></div>
<script type="text/javascript" >
	var listModuleName = "{$MODULE}";
	{literal}

	$("#entry_repertory_box").dialog({
		autoOpen: false,
		height: 300,
		width: 400,
		modal: true,
		...
		dialogClass:"dialog_default_class",
		open:function(){
			var url = "index.php?module=Product&action=entryRepertory";
			$(this).load(url);			
		},
		...
		buttons: {
		"确定":function(){				
				var info = zswitchui_validity_check("#form_entry_view");
				if(info.length>0)
				{
				...
				{
					var url = "index.php?module=Product&action=entryRepertory&operation=entry";
					 zswitch_ajax_request(url,"form_entry_view",function(type,data){
						if(type=="success")
						{
							var url = "index.php?module=Product&action=index"; 
							zswitch_load_client_view(url,"client_listview_table_form");
							$("#entry_repertory_box").dialog( "close" );
						}
						else
						{
							info = "<span style='font-weight:bold;line-height:20px;'>商品入库失败,"+data+"</span>" ;
							zswitch_open_messagebox("editview_input_errorr","输入错误",info,400,300);	
						}
					 });
	...
	$("#out_repertory_box").dialog({
		...
		dialogClass:"dialog_default_class",
		open:function(){
			var url = "index.php?module=Product&action=outRepertory";
			$(this).load(url);	
		},
		beforeClose:function(){
		},
		buttons: {
		"确定":function(){
				var info = zswitchui_validity_check("#form_out_view");
				if(info.length>0)
				{
					info = "<span style='font-weight:bold;line-height:20px;'>以下字段输入无效，请核对。</span>" +"<div style='margin-left:25px;'><br/>"+info+"</div>";
					zswitch_open_messagebox("editview_input_errorr","输入错误",info,400,400);							
				...
				{
					var url = "index.php?module=Product&action=outRepertory&operation=out";
					 zswitch_ajax_request(url,"form_out_view",function(type,data){
						if(type=="success")
						{
							var url = "index.php?module=Product&action=index"; 
							zswitch_load_client_view(url,"client_listview_table_form");
							$("#out_repertory_box").dialog( "close" );
						}
						else
						{
							info = "<span style='font-weight:bold;line-height:20px;'>商品出库失败,"+data+"</span>" ;
							zswitch_open_messagebox("editview_input_errorr","输入错误",info,400,300);	
						}
					 });
		...
		$(".client_listview_button").button().click(function(){
			...
			if(oper == 'entry_repertory')
			{
				$("#entry_repertory_box").dialog("open");
			}
			else if(oper == 'out_repertory')
			{
				$("#out_repertory_box").dialog("open");
			}			
		});
	{/literal}	
</script>
