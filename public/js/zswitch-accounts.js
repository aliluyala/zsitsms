var account = {
    ...
    isRepeat : function(vin,recordid,mod){
        $.post("index.php?module=" + mod + "&action=isRepeat","vin="+vin+"&recordid="+recordid,function(result){
    ...
    set : function(acid,mod){
        ...
        open:function(){
            var url ="index.php?module=" + mod + "&action=lastyearview&recordid="+$(this).data("recordid");
            $(this).load(url);
        },
    ...
    isValidity : function(vin,recordid,mod){
        $.post("index.php?module=" + mod + "&action=isRepeat","vin="+vin.val()+"&recordid="+recordid,function(result){
            var info = "";
    ...
    tip_popup : function(accid){
        ...
        $('#detail_add_track_popup_dlg').dialog({
            ...
            open:function(){
                ...
                var url ="index.php?module=AccountTrack&action=addTrackPopup&accountid="+$('#detail_add_track_popup_dlg').data('accountid');
                $(this).load(url);
            },
            beforeClose:function(){
                $(this).dialog("destroy");
                $(this).remove();
				zswitch_load_client_view("index.php?module=Account&action=detailView&recordid="+ $('#detail_add_track_popup_dlg').data('accountid')+"&return_module=Account&return_action=index");
            },
            buttons: {
            "确定":function(){
                $( this ).dialog( "close" );
    ...
    //增加跟踪记录对话框
    add_track_popup : function(accid,trackModule,accModule){
        if(!$('#main_view_client').children().is('#detail_add_track_popup_dlg'))
        {
            var html = '<div id="detail_add_track_popup_dlg"></div>';
            $('#main_view_client').append(html);
        ...
        $('#detail_add_track_popup_dlg').data('accountid',accid);
        $('#detail_add_track_popup_dlg').data('trackmodule',trackModule);
        ...
        $('#detail_add_track_popup_dlg').dialog({
            ...
            open:function(){
                zswitch_show_progressbar($(this),"import_progress");
                var url ="index.php?module="+$('#detail_add_track_popup_dlg').data('trackmodule')+"&action=addTrackPopup&accountid="+$('#detail_add_track_popup_dlg').data('accountid');
                $(this).load(url);
            },
            beforeClose:function(){
            ...
            "确定":function(){
                ...
                {
                    zswitch_show_progressbar($(this),"import_progress");
                    var url = "index.php?module=" + trackModule + "&action=save";
    ...
    zswitch_listview_batch_recycle_dlg : function(listModuleName){
        if(!$('#main_view_client').children().is('#listview_bar_batch_recycle_dlg'))
                {
                    var html = '<div id="listview_bar_batch_recycle_dlg"></div>';
                    $('#main_view_client').append(html);
                }

                $('#listview_bar_batch_recycle_dlg').data('module',listModuleName);
                $('#listview_bar_batch_recycle_dlg').dialog({
                    autoOpen:true,
                    height:200,
                    width:500,
                    modal:true,
                    ...
                    open:function(){
                        zswitch_show_progressbar($(this),"import_progress");
                        var url ="index.php?module="+$(this).data("module")+"&action=batchRecycle";
                        $(this).load(url);
                    },
                    beforeClose:function(){
                        $(this).dialog("destroy");
                    ...
                    buttons: {
                    "确定":function(){
                        var step = $('#listview_bar_batch_recycle_dlg').data("step");
                        if(step == "1")
                        {
                            zswitch_show_progressbar($(this),"dialog_progress_bar");
                            if($("#batch_recycle_query_where").val() == "current_select")
                            {
                                $("#client_listview_table_form").find("[name^=selected_records]:checked").each(function(){
                                    if($("#batch_recycle_select_recordid_list").val() == "")
                                    {
                                        $("#batch_recycle_select_recordid_list").val($(this).val());
                                    }
                                    else
                                    {
                                        $("#batch_recycle_select_recordid_list").val($("#batch_recycle_select_recordid_list").val()+","+$(this).val());
                                    }
                                });
                            }
                            var url = "index.php?module="+$(this).data("module")+"&action=batchRecycle&step=2";

                            $.post(url,$('#batch_recycle_dialog_form').serialize(),function(result){
                                $('#listview_bar_batch_recycle_dlg').html(result);
                            });
                        }
                        else if(step == "2")
                        {
                            var url = "index.php?module="+$(this).data("module")+"&action=batchRecycle&step=3";
    ...
    zswitch_listview_operation_recycle : function(mod,id){
        var container =$('#main_view_client');
        container.find("#dialog_confirm_recycle_record").remove();

        var html = '<div id="dialog_confirm_recycle_record" title="确认恢复" >';
            html += '<p><span class="ui-icon ui-icon-alert" style="float: left; margin: 0 7px 20px 0;"></span>';
            html += '你确定要恢复记录吗？<br/>点击“确定”恢复，点击“取消”忽略。</p></div>';

        container.append(html);
        var dlg = container.find("#dialog_confirm_recycle_record");
        ...
        dlg.dialog({
         ...
         buttons: {
        "确定":function(){
            var url = "index.php?module="+dlg.data('module')+"&action=recycle&recordid="+dlg.data('recordid');
	...
	//江苏理赔
	jiangsu_insurance : function(id){
		 if(!$('#main_view_client').children().is('#Jiangsu_insurance'))
        {
            var html = '<div id="Jiangsu_insurance"></div>';
            $('#main_view_client').append(html);
        }
      // $('#detail_add_track_popup_dlg').data('accountid',accid);
        $('#Jiangsu_insurance').dialog({
            autoOpen:true,
            height:400,
            width:1200,
            modal:true,
            ...
            open:function(){
                //zswitch_show_progressbar($(this),"import_progress");
                var url ="index.php?module=Insurance&action=JiangsuInsurance&id="+id;	 
                $(this).load(url);
            },
            beforeClose:function(){
                $(this).dialog("destroy");
	...
	jiangsu_image_reload : function(){
		 var url = "index.php?module=Insurance&action=JiangsuInsurance&method=imageReload";
...
var handout = {
    ...
    stepPopUp : function(mod,action){
        ...
        $('#detail_step3_popup_dlg').dialog({
            ...
            open:function(){
                zswitch_show_progressbar($(this),"import_progress");
                var url = "index.php?module="+mod+"&action="+action;
    ...
    transfer : function(mod){
        var info = zswitchui_validity_check("#form_edit_view");
        if(info.length<=0){
            handout.stepPopUp(mod,"transfer");
        }
        else{
            info = "<span style='font-weight:bold;line-height:20px;'>以下字段输入无效，请核对。</span>" +"<div style='margin-left:25px;'><br/>"+info+"</div>";
            zswitch_open_messagebox("editview_input_errorr","输入错误",info,400,400);
