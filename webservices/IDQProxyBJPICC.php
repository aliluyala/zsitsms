...
//file_put_contents('/var/www/html/zsitsms/1.txt','1--',FILE_APPEND);
...
require(_ROOT_DIR.'/include/InsDataExtraction/InsDataExtractionBJPicc.class.php');
...
$ide = new InsDataExtractionBJPicc($conf,$cookiePath) ;
...
if($method == 'queryAll')
{ //file_put_contents('/var/www/html/zsitsms/22.html',json_encode($_GET));
...
//file_put_contents('/var/www/html/zsitsms/cache/match/p1_'.$_POST['vin_no'].'.html',json_encode($p1));	
	if(!is_array($pl)) 
	{ 
		$data = $pl;
	}	
...
{
	$data = $ide->getClaimsInfo($_POST['policy_no'],$_POST);		
}
...
elseif(!$data || empty($data['policy_list']))
{
...
// $data['from']='beijing';
// file_put_contents('/var/www/html/zsitsms/cache/match/return_'.$po['policy_no'].'.html',json_encode($data));
