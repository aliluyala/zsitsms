...
class JS_NEW_CPIC_PC
{
    ...
    public $vehicleQueryConfirm = 'http://issue.cpic.com.cn/ecar/ecar/vehicleQueryConfirm';
    public $vehicleQueryValidation = 'http://issue.cpic.com.cn/ecar/ecar/vehicleQueryValidation'; // 交管验证码
     ...
     * 构造函数
     * 参数:
     * @param $config    array      必需。配置 数组
     * @param $cachePath    string   必需。缓存目录 绝对路径
     **/
    ...
    public function login()
    {
        ...
        if ($this->array_get($result, 'authentication', 'false') == 'true') {
            ...
            if ($loginFastResult['authentication'] == "true") {
                ...
                $this->error['errorMsg'] = $this->array_get($result, 'errMsg', '接口登录出错');
                return false;
            }
        }
...
EOF;
        ...
        $beforeVerification = $this->verification;
        $postFileds = array(
            'meta' => array(),
            'redata' => array(
                'vin' => $info['VIN_NO']
        ...
        $response = $this->jsonRequest($this->vehicleQueryValidation, $postFileds);

        if (!empty($this->error) && isset($this->error['errorMsg'])) {
            echo <<<EOF
<script>
alert("{$this->error['errorMsg']}");
</script>
EOF;
            exit();
        }
        // 首次登录成功后的处理：关闭交管车辆查询的窗口，并重新打开获取交管车查询的验证码（避免修改当前算价器的页面的JS去判断是否登录成功）
        // 注：这种作法为当前采用的简便作法。不推荐。！！！
        if (isset($_GET['checkcode']) && !$beforeVerification && !$this->verification) {
            echo <<<EOF
<script>
$('button.ui-button.ui-widget.ui-state-default.ui-corner-all.ui-button-icon-only.ui-dialog-titlebar-close').trigger('click');
$('button#policy_calculate_check_btn').trigger('click');
</script>
EOF;
            exit();
        }
        ...
        return json_encode(array(
            'checkcode' => $response['result']['checkCode'],
            'checkno' => $response['result']['checkNo'],
        ));
    }
    ...
    public function check_data($info = array())
    {
        ...
        $response = $this->jsonRequest($this->vehicleQueryConfirm, $postFileds);
        $code = $this->array_get($response, 'message.code');
        if ($code == 'failed') {
            $this->error['errorMsg'] = $this->array_get($response, 'message.message');;
            return false;
        }
        $result = $response['result'];
        $_SESSION['car_traffic'] = $result;

        $resultArray = array(
            array(
                'carloteququality' => $this->array_get($result, 'wholeWeight'),
                'color' => $this->array_get($result, 'color'),
                'engineno' => $this->array_get($result, 'engineNo'),
                'enrolldate' => $this->array_get($result, 'vehicleRegisterDate'),
                'exhaustcapacity' => $this->array_get($result, 'displacement'),
                'haulage' => $this->array_get($result, 'haulage'),
                'ineffectualdate' => $this->array_get($result, 'ineffectualdate'),
                'lastcheckdate' => $this->array_get($result, 'lastCheckDate'),// 20170318
                'licenseno' => $this->array_get($result, 'carMark'),
                'licensetype' => $this->array_get($result, 'vehicleType'),
                'limitload' => $this->array_get($result, 'limitload'),
                'madefactory' => $this->array_get($result, 'madeFactory'),
                'owner' => $this->array_get($result, 'owner'),
                'pmfueltype' => $this->array_get($result, 'fuleType'),
                'producertype' => $this->array_get($result, 'channelType'),
                'rackno' => $this->array_get($result, 'rackNo'),
                'rejectdate' => $this->array_get($result, 'rejectDate'), //20991231
                'saleprice' => $this->array_get($result, 'salePrice'),
                'seatcount' => $this->array_get($result, 'limitLoadPerson'),
                'status' => $this->array_get($result, 'status'),
                'transferdate' => '',
                'usernature' => '',
                'usetype' => $this->array_get($result, 'useType'),
                'vehiclebarand1' => $this->array_get($result, 'vehicleBrand1'),
                'vehiclebarand2' => $this->array_get($result, 'vehicleBrand2'),
                'vehiclecategory' => '',
                'vehiclemodel' => $this->array_get($result, 'vehicleModel'),
                'vehiclestyle' => $this->array_get($result, 'vehicleStyle'),
            )
        );
...
//            'seatCount' => $seats,                                                        // 核定载客量(座)
            ...
            'emptyWeight' => $_SESSION['car_traffic']['wholeWeight'],                    // 整备质量(千克)现在以交管查询出来的信息为准
            'engineCapacity' => str_replace(',', '.', number_format($_SESSION['car_traffic']['displacement'])),               // 排量(升)
            'seatCount' => $_SESSION['car_traffic']['limitLoadPerson'],                  // 核定载客量(座)

            // !!! 江苏交管车辆查询后 增加的参数
            'stCertificationDate' => '',
            'vehicleStyle' => $_SESSION['car_traffic']['vehicleStyle'],
            'jqVehicleClaimType' => '',
            'syVehicleClaimType' => '',
        );
//        print_r($_SESSION['car_traffic']);
//        print_r($ecarvo);die;
        return $ecarvo;
    }
    ...
    private function buildCompulsory($mvtalci)
    {
        ...
        $compulsoryInsuransVo = array(
            ...
            'taxType' => 'T',                              // 纳税类型 T正常缴税 C减税 E免税 P已完税 R拒缴
            'taxBureau' => '',                             // 税务机关代码
