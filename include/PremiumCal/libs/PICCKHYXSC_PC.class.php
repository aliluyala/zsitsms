...
class PICCKHYXSC_PC
{
	...
	function __construct($config,$cachePath)
	{
		...
		$this->verifyCodeUrl = "http://157.122.153.67:9000/khyx/um/umtuser/ValidCode.do";//验证码
		...
		$this->jyquery = "http://157.122.153.67:9000/khyx/qth/vehicle/jyQuery.do";//通过车架号查询
		...
		if(empty($cachePath))
		{
			$this->cookie_file = dirname(__FILE__).'/sichuan_picckhyxsc_cookie.txt';
		}
		else
		{
			$this->cookie_file = $cachePath.'/sichuan_picckhyxsc_cookie.txt';  //COOKIE文件存放地址
		}
	...
	/**
	 * [needVerification 返回是否需要登录输入验证码]
	 * @dateTime  2017-09-19
	 * @author LiangYuLin
	 * @copyright Sky_Liang
	 * @license   [license]
   ...
   /**
    ...
    {
        $image = $this->requestGetData($this->verifyCodeUrl.'?set='.time());
    	if(empty($image))
    	{
   ...
   /**
							...
							if(empty($resen) || !isset($resen['itemKindTempList']) || !isset($resen['ciPremium']) ){
                                ...
                                return false;
                             	...
                             	$itemList_str="";
                             	 	...
                             	 	foreach($resen['prpReInsureItems'] as $key => $val)
                             	 	{

                             	 		if($val['expireDate']!="")
                             	 		{
                             	 			$itemList_str.="重复投保单号:".$resen['prpReInsureItems'][$key]['policyNo']."<br />";//重复投保单号
		                             	 	$itemList_str.="签单日期:".$resen['prpReInsureItems'][$key]['billDate']."<br />";//签单日期
		                             	 	$itemList_str.="终保日期:".$resen['prpReInsureItems'][$key]['expireDate']."<br />";//终保日期
		                             	 	$itemList_str.="起保日期:".$resen['prpReInsureItems'][$key]['effectiveDate']."<br />";//起保日期
		                             	 	$itemList_str.="车架号:".$resen['prpReInsureItems'][$key]['vin']."<br />";//车架号
		                             	 	$itemList_str.="发动机号:".$resen['prpReInsureItems'][key]['engineNo']."<br />";//发动机号
		                             	 	$itemList_str.="保险公司:".$resen['prpReInsureItems'][key]['insurerCode']."<br />";//保险公司
		                             	 	$itemList_str.="险种信息:"."<br />";
		                             	 	foreach($resen['prpReInsureItems'][$key]['itemList'] as $k =>$v)
		                             	 	{
		                             	 		$itemList_str.= $v['coverageCode']."<br /><br />";
		                             	 	}
	...
	public function login()
	{
		...
		if(isset($this->checkcode) && $this->checkcode !="")
		{
			...
			$login_result = $this->post($this->UrlLogin,$this->loginAarray);
			if($login_result == -1)//登录成功
			{
				$this->login_stutes =  false;
			}
			else
			{
				$this->login_stutes =  true;
			}
		}
	}
	...
	public function queryBuyingPrice($info=array())
	{
        ...
        if(!empty($checkcode) && $info['checkcode'] !="")
        {
            ...
            $this->login();
            if(!$this->login_stutes)
            {
            	$this->error['errorMsg'] = '登录失败，请确认保险公司账号是否正确。';
            	return false;
            }
        }
		$model = '*';
		if(!empty($info['model']) || $info['model']!="")
		{
		...
		$data["licenseno"]="";
		$data["engineno"]="";
		$PostData['page'] = $info['page'];
		$PostData['rows'] = 10;
		$array = $this->requestPostData($this->PurchasePriceUrl.'?'.http_build_query($PostData),$data);
		if($array == -1)
		{
			return false;
		}
		$array = json_decode($array,true);
		if($array['errorTitle'] == '精友车型交互失败' && strstr($array['errorMsg'],'无数据'))
		{
			$array = $this->vin_car($info);
		}
		/*************请求数组****************/
		if(is_array($array) && array_key_exists('rows',$array))
		{
			$retdata = array('total'=>ceil($GLOBALS['PostData']['total']/10),'page'=>intval($page),'records'=>$GLOBALS['PostData']['total'],'rows'=>array());
		...
		else
		{
			...
			else
			{
              ...
              $TDS_result= $this->requestGetData($this->search."?".$model_url.$TDS_model);
              ...
              else
              {
                    ...
                    foreach ($arr['rows'] as $key) {
                        $car['rows'][$i]['vehicleName']=self::trimall($key[1]);
                        $car['rows'][$i]['vehicleId']=self::trimall($key[2]);
                        $car['rows'][$i]['vehicleSeat']=self::trimall($key[4]);
                        $car['rows'][$i]['vehicleAlias']=self::trimall($key[11]);
                        $car['rows'][$i]['vehicleWeight']=self::trimall($key[6]);
                        $car['rows'][$i]['priceP']=self::trimall($key[7]);
                        $car['rows'][$i]['vehicleYear']=self::trimall($key[9]);
                        $car['rows'][$i]['vehicleDisplacement']=self::trimall($key[5]);
                        $i++;
                    }
	...
	public function vin_car($info = array())
	{
		if(isset($info['vin_no']) && !empty($info['vin_no']))
		{
			$parms = array(

						'jyVehicleRequest.resources' => '0524',
						'jyVehicleRequest.currentpageno' => '',
						'operateType' => 'quote',
						'operateFlag' => '',
						'enrollDate' => '',
						...
						'jyVehicleRequest.brandId' => '',
						'jyVehicleRequest.brandName' => '',
						'jyVehicleRequest.vehicleName' => '*',
						'jyVehicleRequest.vinno' => $info['vin_no'],
						'licenseno' => '',
						'jyVehicleRequest.engineno' => '1695907',
						'page' => $info['page'],
						'rows' => 20
			);
			$respon = $this->requestPostData($this->jyquery,$parms);
			if(!empty($respon))
			{
  ...
  return $td_array;
	...
	public function  datas($result,$business,$mvtalci){
		...
		//	"prpCinsureds[0].sex" => $business['DESIGNATED_DRIVER']['DRIVER_SEX'],//性别 1男性   2女性
			...
			"prpCcarShipTax.taxcomcode" => "00828271-9",
			"prpCcarShipTax.taxcomname" => "四川省地方税务局直属分局",
