...
//            $this->error['errorMsg'] = $this->array_get($response, 'message.message', '接口出错');
    ...
    public function premium($auto = array(), $business = array(), $mvtalci = array())
    {
        ...
        if (false === $this->checkInfoVo($response)) {
            return false;
        }
    ...
    public function getPremiumVerifyCode()
    {
        return $_SESSION['premiumVerifyCode'];
    }

    public $premiumVerifyCode = array();

    /**
     * 检查算价结果中是否包含需要输入验证码并存储验证码
     *
     * @param $response
     * @return bool
     */
    public function checkInfoVo($response)
    {
        $checkInfovo = $this->array_get($response, 'result.checkInfoVo');
        if ($checkInfovo['answerFlag'] == '2') { // 需要输入验证码
            $this->verification = true;
            if ($checkInfovo['isTrafficQuestion'] == "Y") {
                $_SESSION['premiumVerifyCode']['mvtalci'] = $checkInfovo['checkCode'];
            }
            if ($checkInfovo['isTrafficQuestion'] == "N") {
                $_SESSION['premiumVerifyCode']['business'] = $checkInfovo['checkCode'];
            }
            return false;
        }
        return true;
    ...
    private function premiumQuery($auto = array(), $business = array(), $mvtalci = array())
    {
        ...
        if (isset($_POST['business_check_code'])) {
            $return['redata']['checkInfoVo'] = array(
                'answerFlag' => 2,
                'checkCode' => '',
                'isTrafficQuestion' => 'N',
                'questionAnswer' => $_POST['business_check_code']
            );
        }

        if (isset($_POST['mvtalci_check_code'])) {
            $return['redata']['checkInfoVo'] = array(
                'answerFlag' => 2,
                'checkCode' => '',
                'isTrafficQuestion' => 'Y',
                'questionAnswer' => $_POST['mvtalci_check_code']
            );
        }
    ...
    private function buildCompulsory($mvtalci)
    {
        ...
        $compulsoryInsuransVo = array(
            ...
            'registryNumber' => '123123',                   // 完税凭证号 ,不允许为空，值可以随便填写
            'cipremium' => null,
            'taxAmount' => null,
            'vehicleClaimType' => "",
        );
