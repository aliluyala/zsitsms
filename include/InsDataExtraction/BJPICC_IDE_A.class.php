...
class BJPICC_IDE_A
{
	...
	{
		$this->config = $config;
		$this->cookie = $cachePath.'cookie_BJPICC.txt';
		$this->currentUser = $config['uid'];
		
		$this->premiumData = $cachePath.'BJPICCPremiumCalculate.dat';
		if(!is_file($this->premiumData))
		{
			copy(dirname(__FILE__).'/BJPICCPremiumCalculate.dat',$this->premiumData);
		}
	}
	...
	private function post($url,$data,$refer,$encodeing = null,$addheader=array())
	{
		...
		$retinfo = curl_getinfo($curl);
		if(curl_errno($curl))
		{  //file_put_contents('/var/www/html/zsitsms/cache/match/errno.txt',curl_errno($curl));
			curl_close($curl);
			return -2;
		}
	...
	{
		if(strstr($curlInfo['url'],'https://10.134.136.48:8888'))
		{
			return false;
		}
		return true;
	...
	{
		$indexURL = 'http://10.134.136.48';
		$loginURL = 'https://10.134.136.48:8888/casserver/login?service=http%3A%2F%2F10.134.136.48%3A80%2Fportal%2Findex.jsp';
		//$refURL   = 'http://10.134.136.48:8000/prpall/business/editRenewalSearch.do';
		$refURL = 'http://10.134.136.48:8000/prpall/business/prepareEdit.do?bizType=PROPOSAL&editType=NEW';
		...
		$retinfo = curl_getinfo($curl);
		//file_put_contents('/var/www/html/zsitsms/cache/match/loginhtml.txt',$htmlStr);
		if(curl_errno($curl) || $retinfo['http_code'] != '200')
		{ //file_put_contents('/var/www/html/zsitsms/cache/match/login.txt','shibai');
			curl_close($curl);
			return false;
		}
		if(strstr($retinfo['url'] , 'http://10.134.136.48/portal/'))
		{ //file_put_contents('/var/www/html/zsitsms/cache/match/login.txt','chenggong');
		...
		else
		{
			...
			if(!curl_errno($curl) && $retinfo['http_code'] == 200 && strstr($retinfo['url'] , 'http://10.134.136.48'))
			{
				...
				if(!curl_errno($curl) && $retinfo['http_code'] == '200' || strstr($retinfo['url'] , 'http://10.134.136.48'))
				{
					...
					$postdata = file_get_contents(dirname(__FILE__).'/BJPICCPremiumCalculate.dat');
					$postdata = extraction_urlparam_replace($postdata,$rplarr);
					file_put_contents($this->premiumData,$postdata);
					curl_close($curl); //file_put_contents('/var/www/html/zsitsms/cache/match/login.txt','dengluchenggong');
					return true;
				}	
			}			
		}
	...
	{		
		$url = "http://10.134.136.48:8000/prpall/custom/customAmountQueryP.do?";
		$refUrl = "http://10.134.136.48:8000/prpall/business/prepareEdit.do?bizType=PROPOSAL&editType=NEW";
	...
	{ 
		...
		$queryURL = 'http://10.134.136.48:8000/prpall/business/selectRenewal.do?pageSize=10&pageNo=1';
		$refURL   = 'http://10.134.136.48:8000/prpall/business/editRenewalSearch.do';
		...
		$ret = array();//file_put_contents('/var/www/html/zsitsms/cache/match/liucheng.html','共'.count($plate_type).'个'."\n",FILE_APPEND);
		foreach($plate_type as $pt)
		{
			...
			{ 
		       // file_put_contents('/var/www/html/zsitsms/cache/match/liucheng.html',"去年保险记录查询有结果<br>",FILE_APPEND);
		        //file_put_contents('/var/www/html/zsitsms/cache/match/1查询结果.html',$htmlstr,FILE_APPEND);
			    $retdata = json_decode($htmlstr,true);
			    if(is_array($retdata) && $retdata['totalRecords'] > 0)
			    {
			    	foreach($retdata['data'] as $row)
			    	{//file_put_contents('/var/www/html/zsitsms/cache/match/liucheng.html',"1<br>",FILE_APPEND);
			    		$policy = array();
			...
			{//file_put_contents('/var/www/html/zsitsms/cache/match/liucheng.html',"去年保险记录查询失败<br>");
				$ret = $htmlstr;
			}	
		}
		return $ret;	
	...
	public function queryOtherPolicyList($info = array())
	{
		...
		*/
		...
		$URL = 'http://10.134.136.48:8000/prpall/business/caculatePremiunForFG.do';
		...
		{	
			$pol = array();	
			$pol['policy_no']   = $srcdata[0]['policyNo'];
			if(!empty($srcdata[0]['carkindCode']) &&
			   array_key_exists($srcdata[0]['carkindCode'],$this->license_type))
			{
				$pol['license_type'] = $this->license_type[$srcdata[0]['carkindCode']];
			}
			else
			{
				$pol['license_type'] = '';
			...
			$pol['license_no'] = $srcdata[0]['licenseNo'];
			
			if(isset($info['license_no']) && mb_strlen($info['license_no'],'UTF8')==7 
			   && mb_strlen($srcdata[0]['licenseNo'],'UTF8')!=7)
			{
				$pol['license_no'] = $info['license_no'];
			}	
			$pol['engine_no']            = $srcdata[0]['engineNo'];
			$pol['insurance_company']    = $srcdata[0]['insurerCode'];
			$pol['end_date_timestamp']   = $srcdata[0]['endDate']['time']/1000-1;
			$pol['start_date_timestamp'] = $srcdata[0]['startDate']['time']/1000;
			$pol['vin_no']               = $srcdata[0]['vinNo'];
	...
	private function queryCIPolicyInfo($policyNo)
	{
		...
		$URL = 'http://10.134.136.48:8000/prpall/business/showCitemKindCI.do?editType=SHOW_POLICY&bizType=POLICY&bizNo='.$policyNo;
		$refURL = 'http://10.134.136.48:8000/prpall/business/browsePolicyNo.do?bizNo='.$policyNo;
		...
		$browseURL = 'http://10.134.136.48:8000/prpall/business/browsePolicyNo.do?bizNo='.$policyNo;
	...
	public function queryLastYearBI($licenseNo)
	{
		...
		//$url = "http://10.134.136.48:8000/prpall/business/selectRenewalPolicyNo.do?licenseNo=\264\250A7K0X9&licenseFlag=1&licenseType=02";
		$refurl = "http://10.134.136.48:8000/prpall/business/prepareEdit.do?bizType=PROPOSAL&editType=NEW";
		$url = "http://10.134.136.48:8000/prpall/business/selectRenewalPolicyNo.do?licenseFlag=1&licenseType=02&licenseNo={$lno}";
		...
		if($ret['totalRecords']>0)
		{
			...
			$refurl = "http://10.134.136.48:8000/prpall/menu/showMenu.do?systemCode=prpall&userCode=A519400237";
			$url = "http://10.134.136.48:8000/prpall/business/editRenewalCopy.do?bizNo={$pno}";
			$htmlstr = $this->get($url,$refurl,'gzip');
			$htmlstr = mb_convert_encoding($htmlstr,'UTF-8','GB2312');		
			$inputs = extraction_html_inputs($htmlstr);
			...
			$url  = "http://10.134.136.48:8000/prpall/business/editCitemCar.do?editType=RENEWAL";
			...
			$url  = "http://10.134.136.48:8000/prpall/business/editCitemKind.do?editType=RENEWAL";
			$url .= "&bizType=PROPOSAL&bizNo={$pno}&riskCode=DAA&applyNo=&startDate={$sdate}";
			$url .= "&endDate={$edate}&startHour={$shour}&endHour={$ehour}&endorType=&taskID_Ppms=&prpallLinkPpmsFlag=";
			...
			$url  = "http://10.134.136.48:8000/prpall/business/editCinsured.do?editType=RENEWAL";
			$url .= "&bizType=PROPOSAL&bizNo={$pno}&riskCode=DAA&applyNo=&startDate={$sdate}";
			$url .= "&endDate={$edate}&startHour={$shour}&endHour={$ehour}&endorType=&taskID_Ppms=&prpallLinkPpmsFlag=";
			$url .= "&operateDate={$operdate}&motorFastTrack=&operatorProjectCode=&reload=&rnd826={$rnd826}";
			$htmlstr = $this->get($url,$refurl,'gzip');
	...
	{
		if(empty($policyNo)) return false;
		$URL = 'http://10.134.136.48:8000/prpall/business/showCitemKind.do?editType=SHOW_POLICY&bizType=POLICY&bizNo='.$policyNo;
		//$refURL = 'http://10.134.136.48:8000/prpall/business/browsePolicyNo.do?bizNo='.$policyNo;
		$refURL = 'http://10.134.136.48:8000/prpall/business/browseRenewalPolicy.do?bizNo='.$policyNo;

		//$htmlstr = $this->get($URL,$refURL,'gzip');	
		$htmlstr = $this->get($refURL,$refURL,'gzip');
		...
		$inputs = extraction_html_inputs($htmlstr);
		 //file_put_contents("/var/www/html/zsitsms/cache/match/12查询商业险$policyNo.html",json_encode($inputs));
		if(!$inputs) return false;	
	...
	private function queryAutoInfo($policyNo)
	{
        ...
        /*
		$URL = 'http://10.134.136.48:8000/prpall/business/showCitemCar.do?editType=SHOW_POLICY&bizType=POLICY&bizNo='.$policyNo;
		$refURL = 'http://10.134.136.48:8000/prpall/business/browsePolicyNo.do?bizNo='.$policyNo;
		
		$htmlstr = $this->get($URL,$refURL,'gzip');
		if(!is_string($htmlstr)) return $htmlstr;
		*/
		//修改开始 liuyong	20170822
		
		$URL = "http://10.134.136.48:8000/prpall/business/editRenewalCopy.do?bizNo=".$policyNo;
		...
		//修改结束
		//file_put_contents('/var/www/html/zsitsms/cache/match/22.html',$htmlstr);
		$this->autoInfo[$policyNo] = array();
	...
	private function queryCinsuredInfo($policyNo)
	{
		...
		if(empty($policyNo)) return true;
        /*
		$URL = 'http://10.134.136.48:8000/prpall/business/showCinsured.do?editType=SHOW_POLICY&bizType=POLICY&bizNo='.$policyNo;
		$refURL = 'http://10.134.136.48:8000/prpall/business/browsePolicyNo.do?bizNo='.$policyNo;
		
		$htmlstr = $this->get($URL,$refURL,'gzip');
        */
		$URL = "http://10.134.136.48:8000/prpall/business/copyProposal.do?bizNo=".$policyNo;
		$htmlstr = $this->get($URL,$URL,'gzip');
		$inputs = extraction_html_inputs($htmlstr);
		...
		$refurl = $url;
		$operdate = date('Y-m-d');
		$rnd80 = date('D M d H:i:s e O Y');
		$url  = "http://10.134.136.48:8000/prpall/business/editCinsured.do?editType=COPY_PROPOSAL";
		$url .= "&bizType=PROPOSAL&bizNo={$policyNo}&riskCode=DAA&applyNo=&startDate=";
		$url .= "&endDate=&startHour=&endHour=&endorType=&taskID_Ppms=&prpallLinkPpmsFlag=";
		$url .= "&operateDate={$operdate}&motorFastTrack=&operatorProjectCode=&reload=&rnd80.={$rnd80}";
		$htmlstr = $this->get($url,$URL,'gzip');
		
	//file_put_contents('/var/www/html/zsitsms/cache/match/'.$policyNo.'.html',$htmlstr,FILE_APPEND);	
		if(!is_string($htmlstr)) return $htmlstr;
		...
		$count = 0;
		while(true)
		{//file_put_contents('/var/www/html/zsitsms/cache/match/'.$policyNo.'.html','0--',FILE_APPEND);
			$itemtag = 'prpCinsureds['.$count.']';			//file_put_contents('/var/www/html/zsitsms/cache/match/'.$policyNo.'.html',$htmlstr,FILE_APPEND);
			if(!array_key_exists($itemtag.'.insuredFlag',$inputs)) break;
			if(empty($inputs[$itemtag.'.insuredFlag']['value'])) break;
			$item = array();
			$item['insured_flag'] = $inputs[$itemtag.'.insuredFlag']['value'];
			$item['insured_type'] = '';//file_put_contents('/var/www/html/zsitsms/cache/match/'.$policyNo.'.html','1--',FILE_APPEND);
			if(array_key_exists($itemtag.'.insuredType',$inputs) 
			   && array_key_exists($inputs[$itemtag.'.insuredType']['value'],$this->insured_type))
			{//file_put_contents('/var/www/html/zsitsms/cache/match/'.$policyNo.'.html','2--',FILE_APPEND);
				$item['insured_type'] = $this->insured_type[$inputs[$itemtag.'.insuredType']['value']];
			}

			if(array_key_exists($itemtag.'.insuredName',$inputs)  )
			{//file_put_contents('/var/www/html/zsitsms/cache/match/'.$policyNo.'.html','3--',FILE_APPEND);
				$item['insured_name'] = $inputs[$itemtag.'.insuredName']['value'];
			}
			
			if(array_key_exists($itemtag.'.unitType',$inputs))
			{//file_put_contents('/var/www/html/zsitsms/cache/match/'.$policyNo.'.html','4--',FILE_APPEND);
				$item['unit_type'] = $inputs[$itemtag.'.unitType']['value'];
			}
			$item['identify_type'] = '';
			if(array_key_exists($itemtag.'.identifyType',$inputs) &&
			   array_key_exists($inputs[$itemtag.'.identifyType']['value'],$this->identify_type))
			{//file_put_contents('/var/www/html/zsitsms/cache/match/'.$policyNo.'.html','5--',FILE_APPEND);
				$item['identify_type'] = $this->identify_type[$inputs[$itemtag.'.identifyType']['value']];
			}
			
			if(array_key_exists($itemtag.'.identifyNumber',$inputs))
			{//file_put_contents('/var/www/html/zsitsms/cache/match/'.$policyNo.'.html','6--',FILE_APPEND);
				$item['identify_number'] = $inputs[$itemtag.'.identifyNumber']['value'];
			}
			
			if(array_key_exists($itemtag.'.insuredAddress',$inputs))
			{//file_put_contents('/var/www/html/zsitsms/cache/match/'.$policyNo.'.html','7--',FILE_APPEND);
				$item['insured_address'] = $inputs[$itemtag.'.insuredAddress']['value'];
			}	
			$this->cinsured[$policyNo][] = $item;
			$count++;
		}		
		//file_put_contents('/var/www/html/zsitsms/cache/match/'.$policyNo.'.html',json_encode($item),FILE_APPEND);
	...
	private function queryPolicyClaimInfo($policyNo)
	{
		...
		$URL = 'http://10.134.136.48:8000/prpall/business/queryClaimsMsg.do?bizNo='.$policyNo;
		$refURL = 'http://10.134.136.48:8000/prpall/business/browsePolicyNo.do?bizNo='.$policyNo;
	...
	public function getPolicyInfo($policyNo,$otherInfo=null)
	{
		if(empty($policyNo)) return false;
		if(!array_key_exists($policyNo,$this->policyList))
		{ //file_put_contents('/var/www/html/zsitsms/cache/match/liucheng.html',"查询$policyNo信息<br>",FILE_APPEND);
		...
		// file_put_contents("/var/www/html/zsitsms/cache/match/$policyNo.html",json_encode($this->policyList[$policyNo]));
	...
	{
		$URL = 'http://10.134.136.48:8000/prpall/vehicle/vehicleQuery.do?pageSize=50&pageNo=1';
	...
	public function queryVehicleByVIN($vin)
	{
		...
		$URL = 'http://10.134.136.48:8000//prpall/business/queryVehicleByPrefillVIN.do?licenseNo=\264\250&enrollDate=1900-01-01';
		$URL .= "&vinNo={$vin}";
