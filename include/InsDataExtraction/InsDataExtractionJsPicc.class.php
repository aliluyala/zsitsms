...
require($finfo['dirname'].'/JSPICC_IDE_A.class.php');
class InsDataExtractionJSPicc
{
	...
	function __construct($config,$cachePath)
	{
		...
		$this->lib = new JSPICC_IDE_A($this->config,$this->cookie);
	 ...
	 * 返回值:失败 false,成功返回数组
	 **/			
	public function queryLastYearBI($licenseNo,$otherInfo = array())
	{
	 ...
	 * 返回值:失败 false,成功返回数组
	 **/
	public function queryCustom($idno,$otherInfo = array())
	{
		return $this->lib->queryCustom($idno);
	}
