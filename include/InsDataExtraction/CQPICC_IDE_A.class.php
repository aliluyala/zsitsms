...
class CQPICC_IDE_A
{
	...
	function __construct($config,$cachePath)
	{
		...
		$this->premiumData = $cachePath.'CQPICCPremiumCalculate.dat';
		if(!is_file($this->premiumData))
		{
			copy(dirname(__FILE__).'/CQPICCPremiumCalculate.dat',$this->premiumData);
		}
	}
	...
	{
		if(strstr($curlInfo['url'],'https://10.134.134.16:8888'))
		{
			return false;
		}
		return true;
	...
	{
		$indexURL = 'http://10.134.134.16';
		$loginURL = 'https://10.134.134.16:8888/casserver/login?service=http%3A%2F%2F10.134.134.16%3A80%2Fportal%2Findex.jsp';
		//$refURL   = 'http://10.134.134.16:8000/prpall/business/editRenewalSearch.do';
		$refURL = 'http://10.134.134.16:8000/prpall/business/prepareEdit.do?bizType=PROPOSAL&editType=NEW';
		...
		if(strstr($retinfo['url'] , 'http://10.134.134.16/portal/'))
		{
		...
		else
		{
			...
			if(!curl_errno($curl) && $retinfo['http_code'] == 200 && strstr($retinfo['url'] , 'http://10.134.134.16'))
			{
				...
				if(!curl_errno($curl) && $retinfo['http_code'] == '200' || strstr($retinfo['url'] , 'http://10.134.134.16'))
				{
					...
					$postdata = file_get_contents(dirname(__FILE__).'/CQPICCPremiumCalculate.dat');
	...
	{		
		$url = "http://10.134.134.16:8000/prpall/custom/customAmountQueryP.do?";
		$refUrl = "http://10.134.134.16:8000/prpall/business/prepareEdit.do?bizType=PROPOSAL&editType=NEW";
	...
	public function queryPolicyList($info = array())
	{
		...
		$queryURL = 'http://10.134.134.16:8000/prpall/business/selectRenewal.do?pageSize=10&pageNo=1';
		$refURL   = 'http://10.134.134.16:8000/prpall/business/editRenewalSearch.do';
	...
	{   
		...
		*/
		...
		$URL = 'http://10.134.134.16:8000/prpall/business/caculatePremiunForFG.do';
	...
	private function queryCIPolicyInfo($policyNo)
	{
		...
		$URL = 'http://10.134.134.16:8000/prpall/business/browseRenewalPolicy.do?bizNo='.$policyNo;
		$refURL = 'http://10.134.134.16:8000/prpall/business/browseRenewalPolicy.do?bizNo='.$policyNo;
		...
		$browseURL = 'http://10.134.134.16:8000/prpall/business/browsePolicyNo.do?bizNo='.$policyNo;
	...
	public function queryLastYearBI($licenseNo)
	{
		...
		//$url = "http://10.134.134.16:8000/prpall/business/selectRenewalPolicyNo.do?licenseNo=\264\250A7K0X9&licenseFlag=1&licenseType=02";
		$refurl = "http://10.134.134.16:8000/prpall/business/prepareEdit.do?bizType=PROPOSAL&editType=NEW";
		$url = "http://10.134.134.16:8000/prpall/business/selectRenewalPolicyNo.do?licenseFlag=1&licenseType=02&licenseNo={$lno}";
		...
		if($ret['totalRecords']>0)
		{
			...
			$refurl = "http://10.134.134.16:8000/prpall/menu/showMenu.do?systemCode=prpall&userCode=A519400237";
			$url = "http://10.134.134.16:8000/prpall/business/editRenewalCopy.do?bizNo={$pno}";
			$htmlstr = $this->get($url,$refurl,'gzip');
			$htmlstr = mb_convert_encoding($htmlstr,'UTF-8','GB2312');		
			$inputs = extraction_html_inputs($htmlstr);
			...
			$url  = "http://10.134.134.16:8000/prpall/business/editCitemCar.do?editType=RENEWAL";
			...
			$url  = "http://10.134.134.16:8000/prpall/business/editCitemKind.do?editType=RENEWAL";
			$url .= "&bizType=PROPOSAL&bizNo={$pno}&riskCode=DAA&applyNo=&startDate={$sdate}";
			$url .= "&endDate={$edate}&startHour={$shour}&endHour={$ehour}&endorType=&taskID_Ppms=&prpallLinkPpmsFlag=";
			...
			$url  = "http://10.134.134.16:8000/prpall/business/editCinsured.do?editType=RENEWAL";
			$url .= "&bizType=PROPOSAL&bizNo={$pno}&riskCode=DAA&applyNo=&startDate={$sdate}";
			$url .= "&endDate={$edate}&startHour={$shour}&endHour={$ehour}&endorType=&taskID_Ppms=&prpallLinkPpmsFlag=";
			$url .= "&operateDate={$operdate}&motorFastTrack=&operatorProjectCode=&reload=&rnd826={$rnd826}";
			$htmlstr = $this->get($url,$refurl,'gzip');
	...
	private function queryBIPolicyInfo($policyNo)
	{
		if(empty($policyNo)) return false;
		$URL = 'http://10.134.134.16:8000/prpall/business/browseRenewalPolicy.do?bizNo='.$policyNo;
		$refURL = 'http://10.134.134.16:8000/prpall/business/browsePolicyNo.do?bizNo='.$policyNo;
	...
	private function queryAutoInfo($policyNo)
	{
		...
		$URL = "http://10.134.134.16:8000/prpall/business/editRenewalCopy.do?bizNo=".$policyNo;
		$htmlstr = $this->get($URL,$URL,'gzip');
		$inputs = extraction_html_inputs($htmlstr);
	...
	private function queryCinsuredInfo($policyNo)
	{
		...
		$URL = 'http://10.134.134.16:8000/prpall/business/showCinsured.do?editType=SHOW_POLICY&bizType=POLICY&bizNo='.$policyNo;
		$refURL = 'http://10.134.134.16:8000/prpall/business/browsePolicyNo.do?bizNo='.$policyNo;
	...
	private function queryPolicyClaimInfo($policyNo)
	{
		...
		$URL = 'http://10.134.134.16:8000/prpall/business/queryClaimsMsg.do?bizNo='.$policyNo;
		$refURL = 'http://10.134.134.16:8000/prpall/business/browsePolicyNo.do?bizNo='.$policyNo;
	...
	{
		$URL = 'http://10.134.134.16:8000/prpall/vehicle/vehicleQuery.do?pageSize=50&pageNo=1';
	...
	{ 
		...
		$URL = 'http://10.134.134.16:8000//prpall/vehicle/queryVehicleByPrefillVIN.do?prpCitemCar.licenseNo=\264\250&prpCitemCar.enrollDate=1900-01-01';
		$URL .= "&prpCitemCar.vinNo={$vin}";
		$htmlstr = $this->post($URL,'',$URL,'gzip');
		$ret = json_decode($htmlstr,true);
		if(!is_array($ret) || !array_key_exists('msg',$ret) || empty($ret['msg']) ) return false;
		$codes = explode(',',trim($ret['msg'],'|'));
		$ret = array();
	...
	{		
		...
		$URL = "http://10.134.134.16:8000/prpall/custom/customAmountQueryP.do?_identifyType=01&_insuredName=&_identifyNumber={$idno}&_insuredCode=";
		$ref = "http://10.134.134.16:8000/prpall/business/prepareEdit.do?bizType=PROPOSAL&editType=NEW";
