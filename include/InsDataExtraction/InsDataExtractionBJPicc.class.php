...
/**
 * 项目：          保险数据抓取功能包
 * 文件名:         InsDataExtractionPicc.class.php
...
require($finfo['dirname'].'/BJPICC_IDE_A.class.php');
class InsDataExtractionBJPicc
{
	...
	{ 
		...
		$this->lib = new BJPICC_IDE_A($this->config,$this->cookie);
	...
	{		
		...
		//file_put_contents('/var/www/html/zsitsms/cache/match/11queryPolicyList查询去年保险记录返回结果.txt',json_encode($policyList)); 
		...
		{			
			foreach($policyList as $po)
			{
				...
				{
					$succ = true;
					//file_put_contents('/var/www/html/zsitsms/cache/match/13_queryPolicyList'.$info['vin_no'].'.txt',json_encode($policyList));
				}					
			}
			
		}
		if($succ) return $policyList_o;
		//file_put_contents('/var/www/html/zsitsms/cache/match/12_queryPolicyList'.$info['vin_no'].'.txt',json_encode($policyList)); 
		$succ = false;
		$modelCa = false;
		$mqcount = 0;
		$vinfo = $info;
